...
import { 
  ...
  AlertCircle, PlayCircle, Loader2, CheckCircle2 
} from 'lucide-react';
...
const TaskCard = ({ task, mode, onClaim, onAbandon, onUpload, onPreview, onCancelUpload }) => {
  ...
  const handleDownload = async (e) => {
    ...
    // 2. ЛОГИКА ДЛЯ PWA НА IPHONE (SHARE API)
    if (isStandalone && isIOS) {
      // ПРОВЕРКА HTTPS (Share API требует SSL)
      if (window.location.protocol !== 'https:') {
        alert("Ошибка безопасности: Сохранение видео в режиме ярлыка работает только через HTTPS. Обратитесь к администратору.");
        return;
      }

      // ПРОВЕРКА ПОДДЕРЖКИ API
      if (!navigator.share || !navigator.canShare) {
        alert("Ваш браузер или версия iOS не поддерживают системное сохранение. Попробуйте открыть Clipsio через обычный Safari.");
        return;
      }
      ...
      try {
        ...
        if (navigator.canShare({ files: [file] })) {
          // МГНОВЕННЫЙ СБРОС ЛОАДЕРА: Чтобы кнопка не залипла, если iOS залагает в меню "Сохранить видео"
          ...
          throw new Error('iOS отклонила файл из-за размера или формата');
        }
      } catch (err) {
        setIsDownloading(false);
        if (err.name !== 'AbortError') {
          alert("Ошибка сохранения: " + err.message);
          // Крайний фоллбек - пробуем открыть в новом окне
          window.open(url, '_blank');
        }
      }
    } else {
      // 3. ЛОГИКА ДЛЯ ПК И ОБЫЧНЫХ БРАУЗЕРОВ
